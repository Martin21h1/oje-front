...
export default function LanguageSelect() {
    ...
    return (// <FormControl sx={{m: 1, minWidth: 80}} size="small" hiddenLabel>
        ...
        <Select
            ...
            variant={'standard'}
        >
            {usersState.native_languages && usersState.native_languages.map((item) => {
                const isSelected = item.id === localStorage.getItem(localStorageKey);
                const menuItemText = isSelected ? '' : item.name;

                return (<MenuItem key={item.name} value={item.id}>
                        <img
                            loading="lazy"
                            width="20"
